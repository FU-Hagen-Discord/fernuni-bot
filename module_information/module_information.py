...
class ModuleInformation(commands.Cog):
    ...
    async def update_loop(self):
        try:
          self.scrapper = Scrapper(self.courses_file)
          self.data = self.scrapper.scrap()
          self.save_data()
        except:
          pass
    ...
    def stg_string_for_desc(self, result):
        desc = f"\n*({result['stg']})*"
        desc += "\n*Es wurden keine Informationen für deinen Studiengang gefunden, daher wird der erste Eintrag angezeigt*" if 'notfound' in result else ""
        return desc
    ...
    async def help(self, channel, stg):
        await channel.send(
            "```" 
    ...
    @commands.command(name="modul")
    async def cmd_modul(self, ctx, arg_cmd="info", arg_stg=None):
      await self.cmd_module(ctx, arg_cmd, arg_stg)

    @commands.command(name="module")
    async def cmd_module(self, ctx, arg_cmd="info", arg_stg=None):
        if not arg_stg:
        ...
        if arg_cmd in ["handbuch", "mhb", "hb", "modulhandbuch"]:
            await self.handbook(ctx.channel, arg_stg)

        elif arg_cmd in ["probe", "leseprobe"]:
            await self.reading_sample(ctx.channel, arg_stg)
        
        elif arg_cmd in ["info"]:
            await self.info(ctx.channel, arg_stg)

        elif arg_cmd in ["load", "aufwand", "zeit", "arbeitszeit"]:
            await self.load(ctx.channel, arg_stg)

        elif arg_cmd in ["mentoriat", "mentoriate", "support"]:
            await self.support(ctx.channel, arg_stg)
        
        elif arg_cmd in ["exam", "exams", "prüfung", "prüfungen"]:
            await self.exams(ctx.channel, arg_stg)

        else:
            await self.help(ctx.channel, arg_stg)
