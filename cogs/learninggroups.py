...
class LearningGroups(commands.Cog):
    ...
    async def cmd_add_member(self, ctx, arg_member: disnake.Member):
        if self.is_group_owner(ctx.channel, ctx.author) or utils.is_mod(ctx):
            await self.add_member_to_group(ctx.channel, arg_member)
    ...
    async def cmd_remove_member(self, ctx, arg_member: disnake.Member):
        if self.is_group_owner(ctx.channel, ctx.author) or utils.is_mod(ctx):
    ...
    @cmd_learninggroup.command(name="members")
    ...
    @cmd_learninggroup.command(name="kick")
    ...
    @cmd_learninggroup.command(name="leave")
